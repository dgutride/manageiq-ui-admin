This is a sample ManageIQ Admin UI skin.
...
A skin is a directory containing images, styles and (js) texts, that can be linked into an Admin UI instance to alter the look of the application, overriding the default theme.
...
Simple! Just run `./link.sh` in the skin directory, optionally supplying the path to the Admin UI directory (ie. something that includes the `client/` and `server/` subdirectories).
